...
Interface.prototype.sendMessage = function(endpointArn, msg, attributes, callback) {
  var self = this
    , convertFn = null;
...
Interface.prototype._broadcastMessage = function(endpoints, message, attributes, callback) {
  var self = this;

  async.each(endpoints, function(endpoint, cb) {
    self.sendMessage(endpoint.EndpointArn, message, attributes, function() {
      cb();
    });
  }, callback);
};
 ...
 * @param {Function}  callback
 */
Interface.prototype.broadcastMessage = function(message, attributes, callback) {
  var self = this;
  if(typeof attributes === 'function') {
    callback = attributes;
    attributes = undefined;
  ...
  this._getUsers(null, function(err, res) {
    ...
    self._broadcastMessage(res.Endpoints, message, attributes, function() {
      async.whilst(function() {
        // Only keep going so long as we have a token
        return (typeof nextToken !== 'undefined' && nextToken !== null);
      }, function(cb) {
        self._getUsers(nextToken, function(err, res) {
          ...
          nextToken = res.NextToken;
          self._broadcastMessage(res.Endpoints, message, attributes, cb);
        });
      }, function(err) {
